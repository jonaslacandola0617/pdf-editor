...
import {
  ...
  MousePointer2, PenLine, RotateCcw, RotateCw, Save, Search, Shapes, Split, Trash2,
  Type, Upload, X,
} from 'lucide-react'
...
export function AllTools() {
  ...
  const sections: Array<{ title: string; subtitle: string; actions: ToolAction[] }> = [
    ...
    {
      title: 'Find & fill',
      subtitle: 'Work with document content.',
      actions: [
  ...
  return (
    <>
      <button
        className={`all-tools-launcher ${open ? 'active' : ''}`}
        type="button"
        title="All Tools"
        ...
        <div className="all-tools-layer" role="presentation">
          ...
          <aside className="all-tools-drawer" aria-label="All Tools">
            <header className="all-tools-header">
              <div>
                <span className="all-tools-eyebrow">PDF FORGE</span>
                <h2>All Tools</h2>
                <p>Everything available in the editor, in one place.</p>
              </div>
              <button className="all-tools-close" type="button" title="Close All Tools" onClick={() => setOpen(false)}>
                <X size={18} />
              </button>
            </header>
                ...
                <section className="all-tools-section" key={section.title}>
                  ...
                  <div className="all-tools-grid">
                    {section.actions.map((action) => {
                      ...
                      return (
                        <button className="all-tools-action" type="button" key={action.label} onClick={action.run}>
            ...
            <footer className="all-tools-footer">
              <span className="all-tools-local-dot" />
              PDFs and OCR stay on this device.
            </footer>
          </aside>
        </div>
      )}
