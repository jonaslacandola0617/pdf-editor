...
export function AdvancedFormWidgetManager({ bytes, currentPage, onBeforeMutate, onApply, onStatus }: Props) {
  ...
  const mutate = async (key: string, status: string, task: () => Promise<ArrayBuffer>) => {
    ...
    try {
      ...
      if (!mounted.current || latestBytes.current !== bytes) return
  ...
  const saveWidget = (field: AdvancedFormFieldInfo, widget: FormWidgetInfo) => {
    ...
    void mutate(key, 'Updating form widget', () => updateFormWidget(bytes, field.name, widget.widgetIndex, {
      geometry: geometryChanged(draft.geometry, widget.geometry) ? draft.geometry : undefined,
      backgroundColor: draft.backgroundColor !== widget.backgroundColor ? draft.backgroundColor : undefined,
      borderColor: draft.borderColor !== widget.borderColor ? draft.borderColor : undefined,
      borderWidth: Number(draft.borderWidth) !== widget.borderWidth ? Math.max(0, Number(draft.borderWidth) || 0) : undefined,
    }))
  }
  ...
  const importJson = async (file: File | undefined) => {
    if (!file || mutationLock.current || loading) return
    mutationLock.current = true
  ...
  return <section className="advanced-form-widget-manager">
    ...
    <div className="advanced-form-toolbar">
      ...
      <button disabled={Boolean(busy) || loading} onClick={saveTabOrder}><Save /> Save tab order</button>
      <button disabled={Boolean(busy) || loading} onClick={() => void exportJson()}><Download /> Export JSON</button>
      <button disabled={Boolean(busy) || loading} onClick={() => importInput.current?.click()}><Upload /> Import JSON</button>
      ...
      {fields.map((field) => {
        ...
        return <div className="advanced-form-row" key={field.name}>
          ...
          {field.type === 'text' && <div className="advanced-text-behavior">
            ...
            <div className="advanced-form-actions"><button disabled={Boolean(busy) || loading} onClick={() => saveText(field)}><Save /> Save text behavior</button></div>
          </div>}
          ...
          {field.widgets.map((widget) => {
            ...
            return <div className="advanced-widget-row" key={key}>
              ...
              <div className="advanced-form-actions"><button disabled={Boolean(busy) || loading} onClick={() => saveWidget(field, widget)}><Save /> Save widget</button></div>
            </div>
          })}
        </div>
      })}
