...
export function AdvancedTools(props: Props) {
  ...
  return <>
    ...
    <button className="soft-btn native-objects-button" title="PDF objects and navigation" onClick={() => setObjectsOpen(true)}>
    ...
    {objectsOpen && <div className="modal-backdrop native-object-backdrop" onMouseDown={() => setObjectsOpen(false)}>
      <section className="native-object-modal" onMouseDown={(event) => event.stopPropagation()} aria-label="PDF objects and navigation">
        <header>
          <div><span className="eyebrow">PDF STRUCTURE & LOCAL ASSETS</span><h2>Objects, navigation & signatures</h2><p>Manage embedded PDF objects, reusable local signatures, page labels, initial-view preferences and local page-image exports.</p></div>
          <button className="icon-btn" title="Close PDF objects and navigation" onClick={() => setObjectsOpen(false)}><X /></button>
        </header>
