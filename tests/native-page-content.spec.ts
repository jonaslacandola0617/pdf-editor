...
test('edits existing native text content including replacement, color, opacity, position and effective font size', async ({ page }, testInfo) => {
  ...
  const row = manager.locator('.text-content-row').filter({ hasText: 'NATIVE CONTENT TEXT 9941' })
  const objectNumber = Number((await row.locator('.native-page-content-meta').innerText()).match(/Text object (\d+)/)?.[1])
  expect(objectNumber).toBeGreaterThan(0)
  await row.getByLabel(`Native page text object ${objectNumber}`).fill('UPDATED NATIVE CONTENT 9941')
  await row.getByLabel(`Native page text color ${objectNumber}`).fill('#cc2244')
  await row.getByLabel(`Native page text opacity ${objectNumber}`).fill('65')
  await row.getByLabel(`Native page text x ${objectNumber}`).fill('20')
  await row.getByLabel(`Native page text y ${objectNumber}`).fill('12')
  const sizeInput = row.getByLabel(`Native page text font size ${objectNumber}`)
