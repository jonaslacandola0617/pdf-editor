...
async function makeFormPdf(path: string) {
  ...
  name.setText('Before')
  name.addToPage(page, { x: 70, y: 690, width: 220, height: 28 })
  ...
  choice.addOptions(['Alpha', 'Beta', 'Gamma'])
  choice.select('Alpha')
...
function pageNumber(page: Page) {
  return page.locator('.floating-nav input')
}

function stageStatus(page: Page) {
  return page.locator('.stage-top-hint')
}
...
test('viewer, search, page organization, merge, extract, annotations, undo/redo and export', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('.doc-title span')).toContainText('3 pages')
  await expect(page.locator('.thumbnail')).toHaveCount(3)

  await page.locator('.floating-nav > button').nth(1).click()
  ...
  await search.fill('SECOND MAGIC 2202')
  ...
  await expect(page.locator('.thumbnail')).toHaveCount(3)
  await search.fill('FIRST MAGIC 1101')
  await search.press('Enter')
  ...
  await expect(page.locator('.doc-title span')).toContainText('5 pages')

  const extractDownloadPromise = page.waitForEvent('download')
  await page.getByRole('button', { name: 'Extract' }).click()
  const modal = page.locator('.modal')
  await expect(modal).toBeVisible()
  await modal.locator('input').fill('1-2')
  await modal.getByRole('button', { name: 'Extract PDF' }).click()
  const extractDownload = await extractDownloadPromise
  const extractedPath = await extractDownload.path()
  if (!extractedPath) throw new Error('Extract download did not produce a file')
  ...
  await dragOnPdf(page, 0.20, 0.52, 0.52, 0.64)
  ...
  await exportedSearch.fill('QA NOTE 8844')
  await exportedSearch.press('Enter')
  await expect(stageStatus(page)).toContainText('1 matching page', { timeout: 20_000 })
...
test('local library and full workspace survive refresh, while explicit close disables auto-resume', async ({ page }, testInfo) => {
  ...
  await expect(stageStatus(page)).toContainText('Saved to local library')
...
test('AcroForm fields can be discovered, edited, saved locally and restored', async ({ page }, testInfo) => {
  ...
  await makeFormPdf(formPdf)
  await openFile(page, formPdf)
  ...
  const choiceField = page.locator('.form-field').filter({ hasText: 'qa_choice' })
  await choiceField.locator('select').selectOption({ label: 'Beta' })

  const colorField = page.locator('.form-field').filter({ hasText: 'qa_color' })
  await colorField.locator('select').selectOption({ label: 'Blue' })
...
test('image-only document is recognized by OCR locally, searchable, and cached', async ({ page }, testInfo) => {
  ...
  await page.setContent(`<!doctype html><html><body style="margin:0;background:white;font-family:Arial,sans-serif">
    <div style="padding:140px 100px;color:#000">
      <div style="font-size:72px;font-weight:700">SCANNED ORBIT 73921</div>
      <div style="font-size:44px;margin-top:60px">Private local OCR verification page</div>
      <div style="font-size:34px;margin-top:45px">Invoice reference ZX-8844</div>
    </div>
  </body></html>`)
    ...
    if (!['GET', 'HEAD', 'OPTIONS'].includes(method)) {
      try {
        ...
        if (url.hostname !== '127.0.0.1' && url.hostname !== 'localhost') {
          outboundWrites.push(`${method} ${request.url()}`)
        }
  ...
  await expect(page.locator('.doc-title span')).toContainText('1 pages')

  const search = page.getByPlaceholder('Find in document')
  await search.fill('ORBIT 73921')
  await search.press('Enter')
  ...
  const cached = await page.evaluate(async () => {
    return await new Promise<{ text: string; words: number; confidence: number }[]>((resolve, reject) => {
      const request = indexedDB.open('pdf-forge-ocr')
      request.onerror = () => reject(request.error)
      request.onsuccess = () => {
        const db = request.result
        const tx = db.transaction('pages', 'readonly')
        const get = tx.objectStore('pages').getAll()
        get.onerror = () => reject(get.error)
        get.onsuccess = () => resolve(get.result.map((r: any) => ({
          text: String(r.text || ''),
          words: Array.isArray(r.words) ? r.words.length : 0,
  ...
  expect(cached.some((record) => record.text.toUpperCase().includes('ORBIT 73921'))).toBeTruthy()
  ...
  await page.reload()
  await expect(page.locator('.app-shell')).toBeVisible({ timeout: 15_000 })
  const cachedSearch = page.getByPlaceholder('Find in document')
  await cachedSearch.fill('ORBIT 73921')
  const started = Date.now()
  await cachedSearch.press('Enter')
  await expect(stageStatus(page)).toContainText('1 matching page', { timeout: 20_000 })
