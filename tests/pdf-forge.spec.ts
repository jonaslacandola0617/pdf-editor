...
test('viewer, search, page organization, merge, extract, annotations, undo/redo and export', async ({ page }, testInfo) => {
  ...
  const mergePalette = page.getByRole('dialog', { name: 'Quick Actions' })
  await mergePalette.getByPlaceholder('Name the task…').fill('merge')
  await mergePalette.getByRole('button', { name: /Merge documents/ }).click()
  await (await mergeChooser).setFiles(mergePdf)
  await expect(page.locator('.thumbnail')).toHaveCount(5, { timeout: 20_000 })

  const extractPromise = page.waitForEvent('download')
  await page.keyboard.press('Control+k')
  const extractPalette = page.getByRole('dialog', { name: 'Quick Actions' })
  await extractPalette.getByPlaceholder('Name the task…').fill('extract')
  await extractPalette.getByRole('button', { name: /Extract pages/ }).click()
