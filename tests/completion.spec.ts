...
test('selected annotations use one blue frame and eight-point resizing', async ({ page }, testInfo) => {
  ...
  if (!selectedBox) throw new Error('Selected rectangle bounds unavailable')
  await page.mouse.move(selectedBox.x + selectedBox.width / 2, selectedBox.y + selectedBox.height / 2)
  await page.mouse.down()
  await page.mouse.move(selectedBox.x + selectedBox.width / 2 + 80, selectedBox.y + selectedBox.height / 2 + 60, { steps: 8 })
  await page.mouse.up()
